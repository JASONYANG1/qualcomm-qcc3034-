...
static uint8 getSeidNegotiatedFromA2dpLib(uint8 deviceId, uint8 streamId)
{
    uint8 seid = INVALID_SEID;
    a2dp_codec_settings *codec_settings = A2dpCodecGetSettings(deviceId, streamId);
...
static uint8 getSeidFromA2dpLinkData(uint8 deviceId)
{
    uint16 a2dp_index = 0;

    if (getA2dpIndex(deviceId, &a2dp_index))
        return getA2dpLinkDataSeId(a2dp_index);
    else
        return INVALID_SEID;
}
...
{
    uint8 previous_seid = getSeidFromA2dpLinkData(deviceId);
    uint8 current_seid = getSeidNegotiatedFromA2dpLib(deviceId, streamId);

    if ((previous_seid != INVALID_SEID) && (current_seid != INVALID_SEID))
    {
        if (current_seid == previous_seid)
        {
...
static void handleA2dpReconfigureInd(uint8 deviceId, uint8 streamId)
{
    ...
    a2dp_stream_state a2dp_state = A2dpMediaGetState(deviceId, streamId);

    if (a2dp_state == a2dp_stream_open)
    {
