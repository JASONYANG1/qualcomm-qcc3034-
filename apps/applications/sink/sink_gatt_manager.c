...
#endif
#ifdef GATT_GAA_MEDIA_SERVER
                sinkGaaMediaServerConnectEvent(cfm->cid);
#endif                
...
#ifdef GATT_AMA_SERVER
            GATT_MANAGER_INFO(("BLE AMA Discon Ev cid  %04x res %04x  \n",ind->cid,  ind->status ));
            sinkAmaServerDisconnectEvent(ind->cid, ind->status);
#endif
#ifdef GATT_GAA_COMM_SERVER
            SinkGaaCommServerDisconnectEvent(ind->cid, ind->status);
#endif
#ifdef GATT_GAA_MEDIA_SERVER
            sinkGaaMediaServerDisconnectEvent(ind->cid);
#endif
        }
