...
#define APTX_AD_EMBEDDED_SERVICE_CAPABILITY_TWS \
    AVDTP_SERVICE_MEDIA_CODEC, \
    length_of_aptx_adaptive_cap, \
    AVDTP_MEDIA_TYPE_AUDIO << 2, \
    AVDTP_MEDIA_CODEC_NONA2DP,\
...
const uint8 tws_aptx_ad_caps[62] =
{
    ...
    APTX_AD_EMBEDDED_SERVICE_CAPABILITY_TWS
};
...
{
    UNUSED(ttp_latencies);
    uint8 *aptx_adaptive_codec_caps = getStartOfCodecSpecificInformation();
    aptx_adaptive_codec_caps[aptx_ad_ll_ttp_min_offset] = (uint8) (APTX_AD_MIN_LL_TTP_5G | APTX_AD_MIN_LL_TTP_2G4);
    aptx_adaptive_codec_caps[aptx_ad_ll_ttp_max_offset] = (uint8) (APTX_AD_MAX_LL_TTP_5G | APTX_AD_MAX_LL_TTP_2G4);
}
...
static void updateCapabilityAptxAdGenericSupport(bool enabled)
{
    uint8 *aptx_adaptive_codec_caps = getStartOfCodecSpecificInformation();

    if (enabled)
    {
        aptx_adaptive_codec_caps[aptx_ad_sample_rate_offset] |= aptx_ad_generic_supported;
    }
    else
    {
        aptx_adaptive_codec_caps[aptx_ad_sample_rate_offset] &= ~aptx_ad_generic_supported;
...
bool sinkA2dpAptxAdUpdateServiceCapWhenEnteringTwsMode(void)
{
    ...
    /* We need to disable generic AOSP support for aptx adaptive */
    updateCapabilityAptxAdGenericSupport(FALSE);
...
{
    /* We need to re-enable generic AOSP support for aptx adaptive */
    updateCapabilityAptxAdGenericSupport(TRUE);
