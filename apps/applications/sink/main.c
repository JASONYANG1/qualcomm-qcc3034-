...
/* BLE Messages */
        ...
        {
            const CL_DM_BLE_SET_ADVERTISING_PARAMS_CFM_T *cfm = 
                (const CL_DM_BLE_SET_ADVERTISING_PARAMS_CFM_T *)message;

            MAIN_DEBUG_L1(("CL_DM_BLE_SET_ADVERTISING_PARAMS_CFM  [%x]\n ", cfm->status));
...
#ifdef ENABLE_FAST_PAIR 
            /* When BLE master is discovering GATT services & initializing it, accept out of range connection parameters sent by BLE master if audio streaming is not active */
            if(sinkBleGapIsSlaveGattServiceDiscoveryInProgress() && !(sinkA2dpIsA2dpSinkPlaying(a2dp_primary) || sinkA2dpIsA2dpSinkPlaying(a2dp_secondary)))
            {
                accept = TRUE;
            }
#endif
    ...
    switch ( id )
    {
      ...
      case EventSysCodecAptxAdaptiveInUse:
        ...
        case EventUsrVA6:
            ...
            break;
        case EventUsrEnterVaAudioTuningMode:
            SinkVaEnterAudioTuning();
            break;
        case EventUsrExitVaAudioTuningMode:
            SinkVaExitAudioTuning();
            break;
