...
#error "Advertising GAA and AMA support together is not supported" 
#endif

#if defined GATT_GAA_COMM_SERVER && !defined ENABLE_GAA
#error GATT_GAA_COMM_SERVER defined without ENABLE_GAA
#endif
...
#ifdef ENABLE_GAA
#include "gaa/sink2gaa.h"
#endif

#ifdef GATT_GAA_COMM_SERVER
#define GAA_SERVICE_UUID (0xFE26)
#define GAA_SERVICE_DATA_LENGTH (6)
#endif
...
static uint8* bleSetAmaServiceData(uint8 *ama_service_data, uint8 *space)
{
    ...
    {     
        ...
        {
            WRITE_AD_DATA(ama_service_data, space, (uint8)AMA_DEVICE_STATE_DISCOVERABLE);            /*Device State bit mask.  Bit 1: 1, if classic bluetooth is discoverable*/
        }
        else
        {
            WRITE_AD_DATA(ama_service_data, space, (uint8)AMA_DEVICE_STATE_NON_DISCOVERABLE);
        }
        
        if (sinkAccesoryIsEnabled() && (AudioConfigIsVoiceTriggerEnabled()) && (stateManagerGetState() == deviceConnDiscoverable))
        {
            WRITE_AD_DATA(ama_service_data, space, (uint8)(AMA_ACCESSORY_PREFERRED | AMA_RFCOMM_PREFERRED));
        }
        else
        {
...
/*******************************************************************************
NAME
    bleSetGaaServiceData

DESCRIPTION
    Setup GAA Service data

RETURNS
    Pointer beyond GAA service data
*/
static uint8 *bleSetGaaServiceData(uint8 *service_data, uint8 *space)
{
    if (*space >= GAA_SERVICE_DATA_LENGTH + 1)
    {
        uint32 model_id;

        PanicFalse(GaaGetModelId(&model_id));

        BLE_AD_INFO(("GAA Scan Response ["));

        WRITE_AD_DATA(service_data, space, GAA_SERVICE_DATA_LENGTH);
        WRITE_AD_DATA(service_data, space, ble_ad_type_service_data);

        WRITE_AD_DATA(service_data, space, GAA_SERVICE_UUID & 0xFF);
        WRITE_AD_DATA(service_data, space, GAA_SERVICE_UUID >> 8);
...
#ifdef GATT_GAA_COMM_SERVER
    srd_head = bleSetGaaServiceData(srd_head, &space);
#endif
...
uint8 *bleAddPrefixedName(const uint8 *name, uint16 *size_name)
{
    char *prefix = "LE-";
...
#if defined(GATT_GAA_COMM_SERVER) || defined(GATT_AMA_SERVER)
    local_name = bleAddPrefixedName(local_name, &size_local_name);
#endif
