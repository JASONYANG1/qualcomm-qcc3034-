...
static void getAmaFeatureConfig(va_audio_mgr_feature_config_t *feature_config)
{
    ...
    if(configManagerGetReadOnlyConfig(SINK_AMA_CONFIG_BLK_ID, (const void **)&ama_config_data))
    {
        ...
        if(ama_config_data->configure_opus_encoder)
        {
            AudioConfigSetOpusCodecCapability(audio_opus_codec_capability_32kbps);
        }
        else
        {
            AudioConfigSetOpusCodecCapability(audio_opus_codec_capability_16kbps);
        }
...
static void getAmaAudioConfig(va_audio_mgr_audio_config_t *audio_config)
{
    ...
    memmove(&audio_config->mic_config, mic_params, sizeof(voice_mic_params_t));

    /*Wake-word enabled use msbc codec else opus codec */
    if(AudioConfigIsVoiceTriggerEnabled())
    {
        AMA_INFO(("Voice trigger enabled \n"));
    ...
    {
        AMA_INFO(("Voice trigger disabled \n"));
        audio_config->codec_config.codec_type = va_audio_mgr_codec_opus;
    }

    AMA_INFO(("codec %d\n", audio_config->codec_config.codec_type));
