...
static bool sinkInquiryResultIsFirstPeerDevice(remote_device peer_device, CL_DM_INQUIRE_RESULT_T* result);
#endif
...
static bool sinkInquiryResultIsFirstPeerDevice(remote_device peer_device, CL_DM_INQUIRE_RESULT_T* result)
{
    return sinkInquiryIsInqSessionPeer() &&
           (peer_device != remote_device_nonpeer) &&
           BdaddrIsSame(&result->bd_addr, &GINQDATA.inquiry.results[0].bd_addr);
}
#endif /* ENABLE_PEER*/
...
#ifdef ENABLE_PEER
                if(sinkInquiryResultIsFirstPeerDevice(peer_device, result))
                {
                    /* Found peer and it's first on the list (i.e. highest RSSI) */
                    INQ_DEBUG(("INQ: Found peer. Try to connect\n"));
                    inquiryConnectFirst();
                }
#endif
            }
