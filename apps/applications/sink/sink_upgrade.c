...
#ifdef ENABLE_GAA
#define GAA_OTA_RESTARTED_SLC_DELAY D_SEC(3)
#endif /* ENABLE_GAA */
...
#ifdef ENABLE_GAA
                    else if (GaaRebootDueToOtaUpgrade())
                    {
                        UPGRADE_INFO(("GAA: sending EventUsrPowerOn\n"));
                        MessageSend(&theSink.task, EventUsrPowerOn, NULL);
                        MessageSendLater(&theSink.task, EventUsrEstablishSLC, NULL, GAA_OTA_RESTARTED_SLC_DELAY);
                        sinkBlePowerOnEvent();
                    }
#endif /* ENABLE_GAA */
                    else
